...
export function createClientApplet() {
  ...
  function render(state = {}) {
    ...
    root.querySelectorAll('[data-command]').forEach(button => {
      ...
      button.disabled = !enabled
        || ship.energy < COMMANDS[name].cost
        || (name === 'beginAlignment' && (!ship.speedMatched || ship.aligning));
    });
    root.querySelector('[data-nudges]').hidden = !ship.aligning;
    root.querySelectorAll('[data-x]').forEach(button => {
      button.disabled = !enabled || ship.energy < COMMANDS.nudge.cost;
    });
  ...
  return {
    init({ refDoc, appletOperation, state }) {
      ...
      root.querySelectorAll('[data-cost]').forEach(element => {
      ...
      async function command(commandName, args = {}) {
        ...
        try {
          const result = await appletOperation.send('Human command', {
            command: commandName,
            args,
          });
          $('[data-feedback]').textContent = result.status === 'failed'
      ...
      $('form').onsubmit = event => {
        ...
        command('setRotationSpeed', { value: Number($('input').value) });
      };
      $('[data-command="beginAlignment"]').onclick = () => {
        command('beginAlignment');
      };
      $('[data-nudges]').onclick = event => {
      ...
      keys = event => {
        ...
        if (move) {
          event.preventDefault();
          command('nudge', { x: move[0], y: move[1] });
        }
      };
      abort = new AbortController();
      document.addEventListener('keydown', keys, { signal: abort.signal });

      $('[data-radio]').onclick = () => {
        const player = $('#radioPlayer');
        player.hidden = !player.hidden;
        $('[data-radio]').textContent = player.hidden
          ? 'Expand radio · Play'
          : 'Collapse radio · Stop';
        ...
        if (!player.hidden) {
          ...
          iframe.src = 'https://www.youtube.com/embed/1GhGN14R7Jg?autoplay=1';
          iframe.title = 'Cabin radio';
