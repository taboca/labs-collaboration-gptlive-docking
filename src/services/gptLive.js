...
export class OpenAILiveService {
  constructor({ apiKey, client, connectSideband } = {}) {
    this.client = client || (apiKey ? new OpenAI({ apiKey, maxRetries: 0 }) : null);
  ...
  async start(sdp, { execute, status }) {
    ...
    if (this.starting || this.sideband) throw new Error('Channel already started');
    const generation = ++this.generation; this.starting = true;
    try {
      const result = await this.client.live.create({ session: liveSession, transport: { type: 'webrtc', sdp } }, { maxRetries: 0 });
      if (!result.session?.id || !result.transport?.sdp) throw new Error('Incomplete Live session answer');
      const sideband = this.connectSideband(result.session.id);
      if (generation !== this.generation) {
        sideband.socket.once('open', () => { sideband.send({ type: 'session.close' }); sideband.socket.close(); });
        throw new Error('Mission ended during channel creation');
      }
      this.sideband = sideband; this.calls = new Set(); this.execute = execute; this.status = status;
      sideband.on('event', event => { this.handle(event, generation).catch(error => status(`Sideband error: ${error.message}`)); });
      sideband.on('error', () => status('Sideband error'));
      sideband.socket.on('open', () => status('Sideband connected'));
      sideband.socket.on('close', () => { if (generation === this.generation) status('Sideband closed'); });
      return result;
    } finally { if (generation === this.generation) this.starting = false; }
  }
  async handle(envelope, generation = this.generation) {
    if (generation !== this.generation) return;
    if (envelope.type === 'session.closed') { this.status('closed'); return; }
    if (envelope.type === 'error') { this.status(envelope.error?.message || 'Live error'); return; }
    const event = envelope.event;
    if (envelope.type !== 'response.event' || event?.type !== 'response.output_item.done' || event.item?.type !== 'function_call') return;
    ...
    try {
      ...
      if (!commands[item.name] || !args || Array.isArray(args) || typeof args !== 'object' || Object.keys(args).length)
        throw new Error('Unsupported tool or arguments');
      result = await this.execute(commands[item.name]);
    } catch (error) { result = { status: 'failed', reason: error.message }; }
    if (generation !== this.generation) return;
    this.send({ type: 'response.item.create', event_id: randomUUID(), item: {
  ...
  send(event) {
    if (this.sideband?.socket.readyState !== 1) throw new Error('Live sideband is not connected');
  ...
  close() {
    ...
    if (this.sideband?.socket.readyState === 1) {
      try { this.sideband.send({ type: 'session.close' }); } catch {}
    }
    const socket = this.sideband?.socket;
    if (socket?.readyState === 0) {
      const sideband = this.sideband;
      socket.once('open', () => {
        try { sideband.send({ type: 'session.close' }); } finally { socket.close(); }
      });
    }
    else socket?.close();
    this.sideband = null; this.calls?.clear();
  }
}
